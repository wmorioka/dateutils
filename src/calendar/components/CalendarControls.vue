<script setup>
defineProps(['isTwoMonthsModeEnabled', 'datePickerMode'])
</script>
        ...
        <div class="flex items-center">
            <template v-if="datePickerMode">
                <button id="date-picker-now" type="button" title="Select now" @click="$emit('now')"
                    ...
                    {{ $t('calendar.controls.now') }}
                </button>
            </template>
            <button type="button" class="calendar-prev-button" :title="$t('calendar.controls.prev')" @click="$emit('move', 'prev')">
            ...
            <button type="button" class="calendar-next-button" :title="$t('calendar.controls.next')" @click="$emit('move', 'next')">
                <svg class="fill-gray-500 w-7 h-7" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
            ...
            <template v-if="!datePickerMode">
                <button id="switch-one" type="button" :title="$t('calendar.controls.showOneMonth')" @click="$emit('toggleMode', false)"
                    :class="{ 'switch-selected': !isTwoMonthsModeEnabled }"
                ...
                <button id="switch-two" type="button" :title="$t('calendar.controls.showTwoMonths')" @click="$emit('toggleMode', true)"
                    :class="{ 'switch-selected': isTwoMonthsModeEnabled }"
